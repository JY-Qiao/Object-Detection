...
def yolo_body(model_input, num_anchors, num_classes):
    ...
    x1 = CBL5(out[2],512)
    feat1_output = CBLC(x1,512,num_anchors*(num_classes+5))

    x = CBLU(x1,256)
    x = Concatenate()([x,out[1]])

    x2 = CBL5(x,256)
    feat2_output = CBLC(x2,256,num_anchors*(num_classes+5))

    x = CBLU(x2,128)
    x = Concatenate()([x,out[0]])

    x3 = CBL5(x,128)
    feat3_output = CBLC(x3,128,num_anchors*(num_classes+5))

    model = Model(inputs=model_input, outputs=[feat1_output, feat2_output, feat3_output])
