...
def get_y_pres(y_pres, input_size, anchors):
    # .shape = [1,20,20,255] and [1,40,40,255] and [1,80,80,255]
    ratio = input_size / int(y_pres.shape[1])
    y_pres = np.reshape(y_pres, [y_pres.shape[-3], y_pres.shape[-2], 3, y_pres.shape[-1] // 3])  # resize为[20,20,3,85]，以20为例
    # 获得网格点坐标
...
def draw_result(boxes, img, class_names):
    ...
    for box in boxes:
        xmin = box[0] * img.size[0] / 640
        ymin = box[1] * img.size[1] / 640
        xmax = box[2] * img.size[0] / 640
        ymax = box[3] * img.size[1] / 640
...
def main():
    ...
    model = yolo_body(model_input, 3, len(classes_name), size)
    print('已加载模型')
    # 加载权重文件
    ...
    while True:
        ...
        try:
            ...
            image = img.resize(size=(640, 640))
            image = np.array(image, dtype='float32')
            image /= 255.
            image = np.expand_dims(image, axis=0)  # 将图片数据转换为[1,640,640,3]的形式
            # 获得预测结果
            ...
            idx = 0
            for y_pre in y_pres:
                # y_pre的形状为[1,20,20,85*3] or [1,40,40,85*3] or [1,80,80,85*3]]
                new_y_pre = get_y_pres(y_pre, 640, anchors[idx])
                # new_y_pre的形状为[20,20,3,85] or [40,40,3,85] or [80,80,3,85]
