...
# 从保存好annotations的txt文件中读取annotation
with open(dataset_txt_path) as f:
    annotations = f.readlines()

# 设置训练参数
num_anchors = 3  # 一个grid中需要预测的三个框
input_size = 640  # 将输入图片的尺寸定义为 640*640
batch_size = 2
...
model_body = yolo_body(model_input, num_anchors, num_classes, size)
print('已构建模型')

y_true = [Input(shape=(input_size // {0: 32, 1: 16, 2: 8}[l], input_size // {0: 32, 1: 16, 2: 8}[l], num_anchors, num_classes + 5)) for l in range(3)]
...
# 设置冻结层数
freeze_layers = {'s': 125, 'm': 179, 'l': 234, 'x': 290}[size]
