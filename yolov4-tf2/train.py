...
model_body = yolo_body(model_input, num_anchors, num_classes)
print('已构建模型')

y_true = [Input(shape=(416 // {0: 32, 1: 16, 2: 8}[l], 416 // {0: 32, 1: 16, 2: 8}[l], num_anchors, num_classes + 5)) for l in range(3)]
...
for layer in range(freeze_layers):
    model_body.layers[layer].trainable = False
...
# 解冻后训练
for layer in range(freeze_layers):
    model_body.layers[layer].trainable = True
# 预热世代
