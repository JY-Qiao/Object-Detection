...
def get_y_trues(boxes, anchors, box_shape, head_shape, input_size, classes, label_dataset):
    ...
    for box in boxes:
        xmin, ymin, xmax, ymax, class_idx = box[0], box[1], box[2], box[3], box[4]
        xmin, ymin, xmax, ymax, class_idx = int(xmin), int(ymin), int(xmax), int(ymax), int(class_idx)
...
def data_generator(annotations, head_shape, box_shape, classes, anchors, input_size, batch_size):
    ...
    while True:
        ...
        for time in range(times):
            image, boxes = get_Mosaic_data(annotations[time*4:(time+1)*4], (input_size, input_size))
        ...
        yield image_dataset, label_dataset
...
# input_size = 416
# print(data_generator(dataset_txt_path, head_shape, box_shape, classes, anchors, input_size))